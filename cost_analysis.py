...
path_to_latex = None # Set this to the path of your LaTeX installation if you want to use LaTeX rendering in matplotlib
# If LaTeX is available, enable it in matplotlib
if path_to_latex:
...
# get orig. img filename from media object
def get_img_filename(mo):
    media = next((v for v in medias if v.id == mo.media_id), None)
    if media is None:
        raise ValueError(f"Media object {mo.id} has no media")
    return media.back_reference + ".png"
